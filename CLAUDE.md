...
- **"Model" is reserved for the LLM.** In this repo "model" always means the language model (the thing behind `ModelProvider` / "Model access"). Never use it for the DDD domain-model sense — that collides and confuses every reader. For DDD, write "domain", "domain language", "ubiquitous language", or "sub-domain". Same discipline for other overloaded words: name the specific sense, never the ambiguous one.
