...
2. **`ocu-sandbox` ships an unsigned, unattested image.** `ghcr-guest.yml` publishes to GHCR with zero syft/cosign/SBOM tooling — the only repo of the eight with none. Every sibling supplies the pattern.
...
| Signed SBOM + SLSA L3 provenance | NFR-SEC-07/18, top-3 | **STALE — shipped in 7 of 8.** `release.yml` (filestore, control, rclone, audit-unpushed), `security.yml` (gateway), `ghcr-image.yml` (webui, admin: cosign-sign + SBOM-attest by digest) | Fill **`ocu-sandbox` only** — `ghcr-guest.yml` publishes with no SBOM and no signature. Keystone: an artifact without either fails the release job (required-ABSENT), and `cosign verify` passes against the published digest |
