...
OCU is one component of the Wide-Moat opinionated bundle (other peers in the bundle include n8n and Open WebUI). OCU is also usable standalone — any MCP-speaking peer integrates. See [`manifesto/01-audience-and-buyer.md`](manifesto/01-audience-and-buyer.md) for the buyer story; this document scopes only what is inside OCU and what it talks to.
...
Canonical source: [`diagrams/c4-context.mmd`](diagrams/c4-context.mmd). Convention: solid border = present on the minimal-capability shelf; dashed border = not on the minimal shelf by default. Palette borrows the project red-untrusted / green-trusted convention; semi-trusted (amber) and isolated (blue) zones from the trust-boundary diagram do not apply at the Context level. Per-actor required/optional status is in the §4 table. Internal containers are not shown here.

## 4. External actors

The boundary-crossing actors are defined canonically in [`02-trust-boundaries.md`](02-trust-boundaries.md) §3.
...
| **Customer KMS / HSM** | Key custody for the Control plane's Storage-JWT signing key (full shelf), the credential authority that issues the real filestore credential, and the audit signing chain | optional — full shelf only; minimal shelf uses host-local keys | [NFR-FLEX-04](manifesto/02-nfrs.md) |
...
| **Data-plane client** (OCU SPA or headless caller) | Reaches the Web UI for file preview, upload, list, and download; bytes flow client↔OCU directly, never through a peer and never to the storage engine. May be driven by a human; the operator path is a separate surface — the CLI or the opt-in read-only `ocu-admin` console, not this data plane | optional — absent in headless / automated deployments; OIDC-embed token required on the full shelf | [NFR-SEC-09](manifesto/02-nfrs.md), [NFR-SEC-49](manifesto/02-nfrs.md), [NFR-SEC-73](manifesto/02-nfrs.md), [NFR-SEC-82](manifesto/02-nfrs.md), [NFR-SEC-83](manifesto/02-nfrs.md) |
...
A human drives OCU's control plane only through an MCP-speaking peer (e.g. Open WebUI, n8n, or a custom client) or, for operations, the CLI (`occ`) or the opt-in read-only `ocu-admin` console; there is no mutating operator UI. The file surface is separate: the Web UI is embeddable cross-origin in a calling peer ([NFR-SEC-82](manifesto/02-nfrs.md), [NFR-SEC-83](manifesto/02-nfrs.md)). The calling client's chat/workflow surface is its own.
...
- **Mutating operator console** — v1 non-goal; state-mutating operator operations run over the CLI (`occ`) + GitOps. The read-only `ocu-admin` console (live view of sessions, quota, audit) IS in scope and opt-in; so is the Web UI (file preview / upload / download, §4).
- **AI-guardrail / prompt-content policy** — customer's AI gateway, not OCU ([`02-trust-boundaries.md`](02-trust-boundaries.md) §2 zone 4, §6).
