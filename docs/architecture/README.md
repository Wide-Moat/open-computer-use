...
Source: [`diagrams/architecture-overview.drawio`](./diagrams/architecture-overview.drawio). Box colour marks the subsystem; we build the green, blue, orange, purple and grey components, while yellow and red are off-box or customer-provided. Two request paths are load-bearing: the **agent path** (MCP caller → MCP gateway → control plane → guest sandbox; everything that creates or manages a session enters through the control plane, which dials into the guest) and the **storage path** (the guest mount client and the Web UI both call `ocu-filestore`, the sole entry point to storage, which fronts a pluggable engine — S3 off-box or a local volume on-box). The guest holds only a short-lived, `filesystem_id`-scoped weak session JWT; the egress edge validates it, exchanges it at the credential issuer for the real filestore credential, and injects that toward `ocu-filestore` (the one door, which the edge sits before), where the storage engine enforces the scope.
