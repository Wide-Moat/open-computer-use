...
| File / artifact data plane | Data-plane client → [Web UI](components/08-web-ui.md) | OpenAPI 3.1 (HTTP+JSON: upload/list/metadata/download/delete + additive archive + embeddable SPA) | define | NFR-SEC-78, NFR-SEC-82, NFR-SEC-49, NFR-SEC-73 |
...
- `contracts/openapi/files-api.openapi.yaml` — the north Files-API data plane (the five ADR-0023 verbs plus the additive `GET /v1/files/archive`), served on the object-store service's north listener over the F9 host leg ([ADR-0025](adr/0025-f9-internal-transport.md)), bodies frozen by [ADR-0028](adr/0028-files-api-body-freeze.md) ([#304](https://github.com/Wide-Moat/open-computer-use/issues/304)). Scope is the host-attested `X-OCU-Filesystem-Id` header, so it carries no `servers` block; no credential crosses the leg.
