...
| Control → guest control-RPC | Control / operator API → Session sandbox | HTTP+JSON over a host-owned UDS (closed tagged-union; v1 verb set `shutdown` only) | define | NFR-SEC-43, NFR-SEC-76 |
