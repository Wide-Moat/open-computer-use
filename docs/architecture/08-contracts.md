...
| Storage [data leg](glossary.md#data-leg) | Session sandbox → Object-store service (over the Egress trust-edge) | the in-guest mount client (object-store client + transport, one binary) dials `service_url` guest-out, static `Authorization: Bearer`; scope verified at the storage engine | define | NFR-SEC-25, NFR-SEC-46, NFR-SEC-85 |
