...
| P1-E2 | A2 | Caller invokes a tool or action beyond its authorization | Audience-validated authN bounds who reaches the surface; host-attested identity blocks cross-session addressing downstream; a deny-by-default policy on (caller, tool, arguments) decides the call before dispatch ([ADR-0041](../adr/0041-gateway-per-action-authz-policy.md)) | NFR-SEC-49 |

Per-action authorization (P1-E2) is enforced: a boot-loaded deployment policy ([ADR-0041](../adr/0041-gateway-per-action-authz-policy.md)) decides every call on (caller, tool, arguments) between the tool-name allowlist and the resolve step, so a refused call acquires no session slot. Argument predicates cover the file verbs' path; a shell-class tool is decided by its class grant, and its effects are confined by the sandbox boundary rather than by a command pattern. P2-E1 is specified in the Control/operator API spec; this spec carries only the gateway-side property (no MCP-surface route to a lifecycle/kill-switch op).
