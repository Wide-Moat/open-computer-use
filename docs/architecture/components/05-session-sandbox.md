...
| Control → guest | mount config: one config with top-level `service_url` + `ca_cert_pem` and a `mounts[]` array; each mount carries `destination`, its own `auth_token` (the off-box-issued scoped JWT), the `filesystem_id`/`memory_store_id` scope, and `readonly` (F7) | guest agent (mount client) | written before the in-guest mount client starts; the guest holds no storage credential beyond the per-mount scoped JWTs |
