...
| guest → edge | the one outbound network leg | guest agent | guest sends an unauthenticated request; the route is the only path off the container |
...
| P5-I3 | A1 | Guest tunnels tenant data to an allow-listed destination over default transparent (content-blind) egress. The container's only recovery is a single deny-by-default forward route plus a payload-independent egress-byte / destination-cardinality tripwire emitted to audit and SOAR; deep content inspection runs only on the opt-in MITM leg, owned by the Egress trust-edge ([`06-egress-trust-edge.md`](06-egress-trust-edge.md), F9). Residual: content-blind transparent egress — theme [#182](https://github.com/Wide-Moat/open-computer-use/issues/182). | NFR-SEC-57 |
