...
The mount config is one shape on this push; there is no token-absent variant handed on separately. The per-mount `auth_token` reaches the guest, where the mount client holds it in its mount config and forwards it unmodified as a static `Authorization: Bearer`; scope is enforced at the storage engine. The host writes only the mounts this session is provisioned for, so the config the guest receives carries only its own scoped JWTs.
