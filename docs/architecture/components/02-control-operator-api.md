...
- A SOAR revoke is verified before it is remembered. The signature check runs first, and only a verified issuance enters the anti-replay window and seen-signature set ([ADR-0039](../adr/0039-soar-webhook-signature-scheme.md)); admitting an unverified one would let anyone reaching the operator socket poison that set with a forged future-dated issuance and have the legitimate revoke refused as a replay (P2-R2, NFR-SEC-01).
