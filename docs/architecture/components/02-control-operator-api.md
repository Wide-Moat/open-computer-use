...
5. Operator-auth substrate (PAM-JIT integration, full vs minimal shelf) belongs in a dedicated ADR — needs issue: open a tracking issue for the operator-authentication ADR (cited as `needs ADR:` in Boundaries and Operational concerns).
