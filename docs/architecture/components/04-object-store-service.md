...
Config surface: the `filesystem_id`→prefix map, the engine-adapter selection and its backend endpoint ([ADR-0010](../adr/0010-storage-backend-pluggable-adapter.md)), and the chunk-policy size ceiling (literal defaults in [`08-contracts.md`](../08-contracts.md) §3). The frozen `ProvisionMountConfig` fields arrive over the host→guest provisioning push before the mount client starts ([ADR-0014](../adr/0014-storage-transport-tier-universal-network-leg.md)): `auth_token` (the scoped Storage-JWT), `ca_cert_pem` (the inspection-CA anchor, the provisioning-wire field, not the rclone-local `ca_cert` option the client derives from it), `service_url`, and the per-mount `destination`; the service does not fetch them.
