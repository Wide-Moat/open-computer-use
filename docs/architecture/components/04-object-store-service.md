...
The file-operation verb names are fixed in [`file-ops`](../../../contracts/storage/file-ops.schema.json): `createFile`, `readFile`, `readMetadata`, `getFileMetadata`, `readFileMetadata`, `copyFile`, `moveFile`, `removeFile`, `fileDelete`, `listFiles`, `listDirectory`, `makeDirectory`, `moveDirectory`, `removeDirectory`, `fileUpload`, `fileDownload`, `importFiles`, `importZip`, `migrateFilesystem`, `removeFilesystem`, `releaseQuarantinedFiles`. Each verb carries per-request `{intent, downloadable}` metadata alongside the `filesystem_id` scope; the three authorization axes are fixed and the per-operation message bodies are TBD in the schema. `readFileMetadata` reads metadata by path plus `filesystem_id`, distinct from the path-addressed `readMetadata` and the by-uuid `getFileMetadata`. `fileDelete` deletes by uuid, distinct from the path-addressed `removeFile`. `migrateFilesystem`, `removeFilesystem`, and `releaseQuarantinedFiles` act on a whole filesystem, a filesystem-scope decision distinct from a per-file one ([ADR-0015](../adr/0015-storage-decomposition-by-trust-plane.md)).

This route serves the guest leg — the in-guest mount client. The host leg (the Web UI) reaches the durable handle-store over a separate dedicated north listener speaking the five Files-API verbs, not this mount RPC ([ADR-0025](../adr/0025-f9-internal-transport.md)). The mount-RPC route shape is fixed: each verb is `POST <service_url>/v1/filestore/fs/<operation>`, the operation name as the trailing path segment, the weak session JWT on the `Authorization` header. `fileUpload` carries a `multipart/form-data` body; `fileDownload` returns a chunked `application/octet-stream`; the rest are REST-JSON over HTTP/2. The HTTP status is authoritative for the outcome and a deny carries the shared `DenyReason` (`BoundedReason`) body for diagnostics only — the same envelope the exec and control-RPC surfaces use ([NFR-SEC-51](../manifesto/02-nfrs.md)). Per-operation request/response field sets stay TBD until sourced; the route, the verb set, the three authorization axes, and the deny envelope are frozen.
...
2. No caller request names a backend object directly; the service maps a verb to an engine request bound to the mapped prefix. A caller-supplied scope id is treated as a hint and rejected if it does not match the host-attested `filesystem_id` binding (property-test, [NFR-SEC-43](../manifesto/02-nfrs.md)). On the north face the `filesystem_id` claim is a per-session derived scope; the service authorizes it against the Control-held session binding, not as a whole-namespace grant ([ADR-0030](../adr/0030-per-chat-storage-scope-derivation.md)).
...
5. `downloadable` is resolved at read from the host-attested session, never from a caller-supplied claim; a non-downloadable object is readable in-session but yields no egress-eligible artifact, and `intent=preview` stays read-only regardless of stored tag (property-test, [NFR-SEC-73](../manifesto/02-nfrs.md)). The stored-tag lookup keys on the engine-relative path with no leading slash — `outputs/report.pdf`, never `/outputs/report.pdf` — one convention across the north and south planes ([ADR-0029](../adr/0029-storage-scope-subtree-resolution.md)).
...
7. Every backend file-activity commits an OCSF File System Activity event to a local durable record (fsync) before the operation is acknowledged, then fans the event out to the hash-chained pipeline; the durable commit is the non-repudiation point, so a downstream sink failure does not deny or stall the file operation (fail-open) — a dropped fan-out is counted and reconciled, never silently lost (unit-test, [NFR-SEC-79](../manifesto/02-nfrs.md)).
...
10. The engine joins every file-op path under the intent's subtree before the invariant-1 traversal check: `subtree = map[intent]`, and the resolved backend path is `<filesystem_id prefix>/<subtree>/<path>`, rejected if the cleaned result escapes that join. The intent is the credential's claim, not caller metadata (invariant 2); the deployment `-granted-intents` flag is a ceiling the claim intersects, never a grant. A write-intent credential addressing `uploads/x` lands the distinct object `outputs/uploads/x`, so the read-only subtree is unreachable for writing — the `:ro` posture is engine-enforced, not a guest-mount artifact (property-test on the join and the write-to-read-subtree rejection, [ADR-0029](../adr/0029-storage-scope-subtree-resolution.md), [NFR-SEC-49](../manifesto/02-nfrs.md)).

## Failure modes

Each row traces to one P4-mount STRIDE row in [`06-threat-model.md`](../06-threat-model.md) §3; the Trace column names that row and the controlling NFRs this service carries for it — engine-side and Session-sandbox NFRs on the row's full anchor live in those components. The Recovery column carries the contract. The reaching actor on the file-op path is A1 (in-sandbox guest). The Web UI's failure modes (A2 external data-plane client, embed-token, preview-render, CSRF) are P4-artifact rows in [component 08](08-web-ui.md), not here. Fail-closed is the default on every prefix-resolution boundary; the file-op audit boundary is durable-first fail-open (the event commits to a local durable record before fan-out, a downstream sink failure does not deny the operation), while a privileged operator/SOAR action stays fail-closed (NFR-SEC-45/P7-R3).
...
| Leaked guest bearer replayed against another filesystem | P4-mount-S2 ([NFR-SEC-25](../manifesto/02-nfrs.md) + SEC-31) | The engine rejects the bearer for a foreign `filesystem_id` (HTTP 403 PermissionDenied — a valid signature but a foreign scope; a missing or expired token is 401), so a root read of the mount config yields at most this filesystem for the token's remaining window — not a backend key ([ADR-0013](../adr/0013-storage-credential-custody.md)). Residual: no mid-session refresh, [#267](https://github.com/Wide-Moat/open-computer-use/issues/267). |
| Engine credential disclosed via process compromise / memory scrape | P4-mount-I1 ([NFR-SEC-25](../manifesto/02-nfrs.md) + SEC-33) | The storage-JWT signing key sits at the Control plane and never enters this service or the guest; the worst guest-side disclosure is the scoped, time-bounded bearer, and the host-side engine key is narrowed to a per-session backend credential on the full shelf ([ADR-0013](../adr/0013-storage-credential-custody.md)). Residual: minimal-shelf long-lived host-local engine credential, [#187](https://github.com/Wide-Moat/open-computer-use/issues/187). |
...
The credential authority's JWKS is mounted read-only at `/etc/ocu-filestore/trust/credential-jwks.json`, alongside the existing `/etc/ocu-filestore/tls/` material, and is read once at boot; the service opens no network path to fetch it ([ADR-0042](../adr/0042-engine-credential-verification-anchor.md)). Verification is on by default, and the only unverified posture is a static operator-configured scope bind behind an explicitly insecure-named flag, admitted solely in rigs that stand up the engine with no authority to verify against.

Config surface: the `filesystem_id`→prefix map, the engine-adapter selection and its backend endpoint ([ADR-0010](../adr/0010-storage-backend-pluggable-adapter.md)), the chunk-policy size ceiling (literal defaults in [`08-contracts.md`](../08-contracts.md) §3), the intent→subtree map (default `write→outputs/`, `read→uploads/`, `preview→uploads/`; a deployment overrides the map, never bypasses the join), and the `-granted-intents` ceiling the minted claim intersects ([ADR-0029](../adr/0029-storage-scope-subtree-resolution.md)). The frozen `ProvisionMountConfig` fields arrive over the host→guest provisioning push before the mount client starts ([ADR-0014](../adr/0014-storage-transport-tier-universal-network-leg.md)): `auth_token` (the scoped Storage-JWT), `ca_cert_pem` (the inspection-CA anchor, the provisioning-wire field, not the rclone-local `ca_cert` option the client derives from it), `service_url`, and the per-mount `destination`; the service does not fetch them.

Observability: the OCSF File System Activity stream (invariant 7) plus per-session rate counters, emitted on the audit fan-in flow. The producer is durable-first: the event commits to a local durable record before fan-out, so a downstream sink failure neither denies nor stalls the file operation, and the local durable record is the non-repudiation point; a dropped fan-out is counted and reconciled, never silently lost ([`audit-fanin`](../../../contracts/audit/audit-fanin.asyncapi.yaml), [NFR-SEC-79](../manifesto/02-nfrs.md)). The audit and log surface redacts secrets so a bearer never reaches a log line. The engine's scope denial appears in the stream as an upstream 403, not a service-authored deny event ([ADR-0013](../adr/0013-storage-credential-custody.md)).
