...
It presents a per-host leaf from its own inspection CA, re-originates TLS to the genuine origin, and enforces no storage scope. On the storage leg it validates the guest's weak session JWT (stock `jwt_authn`; missing/invalid → 401), exchanges it at the issuer for the real filestore credential keyed on the validated `filesystem_id` (RFC 8693, per-session, cached), and overwrites the `Authorization` header with that credential before forwarding to the Object-store service ([ADR-0019](../adr/0019-egress-exchanges-filestore-credential.md)); it holds no signing key — it exchanges, never mints.
...
|---|---|---|
| Session sandbox → Egress | the sole outbound path (F8), carrying guest-internet traffic and the mount client's storage leg (F7a) alike | the request carries the caller's own credential (a weak `filesystem_id`-scoped session JWT on the storage leg, an upstream bearer, or none); on the storage leg the edge exchanges that session JWT for the real credential, on the upstream leg it injects only under hardening |
| Egress → Upstream LLM / API | the re-originated request to the genuine origin | the agent's allowed outbound endpoints |
| Egress → Object-store service | the storage leg's re-originated request | the only door to storage; the edge has swapped the guest's weak session JWT for the real filestore credential it injects |
| Egress → Credential issuer | the RFC 8693 token exchange (subject_token = the weak session JWT) | per-session, cached; returns the real filestore credential keyed on `filesystem_id` ([ADR-0019](../adr/0019-egress-exchanges-filestore-credential.md)) |
...
The edge reaches the credential issuer for the token exchange only: it presents the validated weak session JWT and receives the real filestore credential ([ADR-0019](../adr/0019-egress-exchanges-filestore-credential.md)). The real filestore credential's signing key stays at the issuer; the edge holds none. The weak session JWT itself is minted and signed by the control plane, which holds its signing key and publishes the JWKS the edge validates against, and delivers it to the sandbox out of the request path ([ADR-0013](../adr/0013-storage-credential-custody.md)). F9 (Web UI → object-store service) does not cross this hop ([`05-c4-container.md`](../05-c4-container.md) §4).
...
The upstream-credential SDS surface is distinct from the inspection-leaf source and from the storage-leg exchange. The inspection leaf is the per-host certificate the hop presents while terminating TLS. The upstream credential is the bearer the hop attaches on an authenticated upstream leg under edge-injection hardening ([ADR-0016](../adr/0016-egress-baseline-inspection-hop-backend-scope.md)). The real filestore credential is obtained by token exchange at the issuer on the storage leg, not from an SDS source ([ADR-0019](../adr/0019-egress-exchanges-filestore-credential.md)). On the upstream leg the baseline uses no SDS credential source: the caller's own credential passes through and the edge injects nothing.
...
3. On the storage leg the hop swaps the credential: it validates the guest's weak session JWT, exchanges it at the issuer for the real filestore credential, and overwrites the `Authorization` header, holding no storage signing key and minting nothing — it exchanges ([ADR-0019](../adr/0019-egress-exchanges-filestore-credential.md)). On the unauthenticated leg the caller's `Authorization` is forwarded unmodified (swap integration test asserting the forwarded storage-leg `Authorization` differs from the inbound session JWT, and the unauthenticated-leg header is unchanged, [NFR-SEC-23](../manifesto/02-nfrs.md)).
4. Storage scope is not checked at the edge: the edge makes no scope decision (it has already validated the guest JWT and exchanged it for the real filestore credential per Invariant 3), and the storage engine enforces scope on the injected real credential, rejecting a foreign `filesystem_id` with 403 PermissionDenied (a valid signature but a foreign scope; a missing or expired token is 401); the test asserts the 403 originates at the engine ([NFR-SEC-31](../manifesto/02-nfrs.md)).
...
| P6-E2 | A1 | A leaked storage JWT is replayed to reach another filesystem, or a guest forces an unintended cross-scope reach | Scope is enforced at the storage engine: a foreign `filesystem_id` claim is rejected with 403 PermissionDenied — a valid signature but a foreign scope; a missing or expired token is 401 — so a leaked token confines to its own scope for its TTL; egress-side scope binding is the optional hardening | NFR-SEC-31, NFR-SEC-29, NFR-SEC-73 |
...
**Rotation / lifecycle.** On the storage leg the edge caches the exchanged filestore credential per session and re-exchanges at the issuer when the session's window lapses; it holds no signing key and signs nothing, so it has no key to rotate, and the credential's own lifecycle is the issuer's ([ADR-0019](../adr/0019-egress-exchanges-filestore-credential.md), [ADR-0013](../adr/0013-storage-credential-custody.md)). Under edge-injection hardening the edge receives the upstream credential from SDS per-connection and drops it after use; rotation belongs to the SDS source ([NFR-SEC-04](../manifesto/02-nfrs.md)). On sandbox teardown the host-driven finalizer drops the network-bound egress route host-side even if the guest is unresponsive ([NFR-SEC-65](../manifesto/02-nfrs.md)).
