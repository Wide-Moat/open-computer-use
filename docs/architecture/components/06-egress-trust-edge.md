...
4. Storage scope is not checked at the edge: a foreign-`filesystem_id` JWT is forwarded without a scope decision and rejected at the storage engine (403 PermissionDenied — a valid signature but a foreign scope; a missing or expired token is 401); the test asserts the 403 originates at the engine ([NFR-SEC-31](../manifesto/02-nfrs.md)).
