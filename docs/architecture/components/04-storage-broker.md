...
| Broker opens a direct backend dial bypassing the edge | P4-E2 (NFR-SEC-16 + SEC-27) | The bypass dial is refused; a network backend engine's leg must traverse the single egress, while a local-volume engine ([ADR-0010](../adr/0010-storage-backend-pluggable-adapter.md)) opens no network leg to bypass. Residual: transparent-mode egress is content-blind on F9, needing broker-side DLP — content-blind-egress theme, [#182](https://github.com/Wide-Moat/open-computer-use/issues/182). |
