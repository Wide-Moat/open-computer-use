...
The only inbound edge is the audit fan-in (F10, defined in [`05-c4-container.md`](../05-c4-container.md) §4): five host-side components — MCP gateway, Control / operator API, Egress trust-edge, Object-store service, Web UI — emit OCSF events to this pipeline. The Session sandbox does not emit. The only outbound edge is the pluggable sink. The pipeline calls no other component.

### Internal components

```mermaid
flowchart LR
    EXT[5 host-side sources<br/>F10 fan-in, per-source mTLS] -->|OCSF + envelope| ING[Ingest face<br/>verify peer, fairness]
...
Each of the five source channels is a separate mTLS-terminated peer. The pipeline also self-emits on its own channel for compute-metering and saturation events. The Session sandbox is not a source; a guest action is host-authored by the component that observes it (P7-R2).

- **Ingest face** terminates the host-side channels (one address per source), verifies the per-source mTLS peer identity, binds the OCSF `source` to that verified identity, and discards any payload-supplied source claim. The self-emit metering/saturation channel is internally originated, not an mTLS-terminated wire peer. Per-source ingest fairness is applied before admission.
...
The fan-in contract is [`contracts/audit/audit-fanin.asyncapi.yaml`](../../../contracts/audit/audit-fanin.asyncapi.yaml); field types, the shared `MessageEnvelope`, and the OCSF class `$ref`s are fixed there and not restated. The durable-bus substrate is named by role only — the protocol token in the contract is a default binding, the product is an ADR (Open questions). The two self-emitted payloads (compute metering, saturation) carry a stable channel and envelope but an open payload schema — no OCSF v1.x class fits (Open questions).
...
5. **Chain order derives from the per-source monotonic sequence, not wall-clock.** Ordering uses the source's monotonic `sequence`; the wall-clock value is a recorded field, not the ordering key. The host-side trusted-time floor governs the Merkle-head cadence and TTL checks, not chain order. *(red-team clock-rollback harness; [NFR-SEC-48](../manifesto/02-nfrs.md))*
...
P7-S1, P7-R2, P7-D1, and P7-D2 are MITIGATED in [`06-threat-model.md`](../06-threat-model.md) §4; their rows here carry the pipeline's enforcing behaviour and the residual the threat-model parks on its §3.1 P5 rows and its §5 register. Other element rows MITIGATED in §4 are not relisted.

## Operational concerns

The pipeline is the enforcement point for the write-before-ack property of [NFR-SEC-03](../manifesto/02-nfrs.md), [NFR-SEC-45](../manifesto/02-nfrs.md), and [NFR-SEC-72](../manifesto/02-nfrs.md) (system-initiated lifecycle transitions).
