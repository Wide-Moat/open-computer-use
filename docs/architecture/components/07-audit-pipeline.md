...
The pipeline receives over five host-side producer channels — MCP-gateway, Control/operator-API, egress-edge, object-store service, Web UI — each an mTLS-terminated peer, plus the pipeline's own self-emit channel for compute-metering and saturation events. The Session sandbox does not emit: a guest action is recorded host-side by the component that observes it (the egress hop, the object-store service, the control-plane RPC), so a compromised guest can neither reach nor silence this pipeline.
...
Each row traces to one Layer 7 STRIDE row ([`06-threat-model.md`](../06-threat-model.md) §3) by element-ID and repeats that row's controlling NFR. The primary reaching actor is A1, the in-sandbox guest, whose actions reach the pipeline only through the host-side channels that observe it; A2/A3 rows are marked.
