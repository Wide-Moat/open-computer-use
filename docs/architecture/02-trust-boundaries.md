...
Five zones appear as subgraphs on the canonical diagram (§5). Each is a place where our code runs and where a reviewer can point and ask "what crosses this boundary".

Terminology bridge: Compute plane in this doc is what `manifesto/02-nfrs.md` calls `data-plane` (NFR-FLEX-12, NFR-MAINT-02) and `VM` (§02 mermaid). Egress trust-edge is what §02 calls `egress-proxy` (NFR-SEC-05). Layer 3 names supersede §02 short-form labels going forward.
...
| 3 | **Compute plane** | Session sandbox runs here, one per session, ephemeral by default. Container substrate on the minimal-capability shelf; microVM (Kata-FC / Kata-CH) on the full-capability shelf. Guest agent is PID 1. Cross-session network reachability disabled by default per NFR-SEC-22; per-tenant network isolation is a deployment property of this zone. | NFR-SEC-02, NFR-SEC-14, NFR-SEC-22, NFR-FLEX-02, NFR-PERF-02/03 |
| 4 | **Egress trust-edge** | Single outbound path. Network-bound egress identity per NFR-SEC-27 (the fact that a request arrived from the sandbox at all is the identity). Transparent pass-through by default; MITM with customer-CA as an opt-in configuration; DLP-ICAP as a separate opt-in configuration. NFR-SEC-08 "egress gateway" lives here as the MCP allow-list enforcer. Default-deny outbound to Wide-Moat-controlled endpoints per NFR-SEC-16. | NFR-SEC-05, NFR-SEC-08, NFR-SEC-16, NFR-SEC-17, NFR-SEC-27, NFR-FLEX-15, NFR-COMP-26, NFR-COMP-28 |
| 5 | **Audit pipeline** | Durable bus + hash-chained store + bridges to customer sinks. Separate zone because its retention floor, RPO, and tamper-evidence properties differ from control plane. Compute-time metering (NFR-COST-05) emits as audit events on this pipeline. | NFR-SEC-03, NFR-REL-12, NFR-REL-03, NFR-COMP-01, NFR-COST-05, NFR-MAINT-AUDIT-SCHEMA |

Reserved as a TBD-stub per `MANIFESTO.md` non-goals: **Skill registry boundary**. Skills are deferred for v1; the boundary is named here so the contract is ready when skills land.

Cross-component encryption-in-transit invariant per NFR-SEC-37: every inter-zone arrow is encrypted in transit; the two documented carve-outs (MITM-proxy when MITM-tier active; DLP-ICAP hook) are the only plaintext segments and are listed explicitly in §7.

## 3. External actors

External actors appear on the diagram but are not zones we own. We document the contract that crosses each boundary; we do not implement the actor.
...
| Transparency log (external; daily Merkle head) | Audit pipeline → transparency log | yes — RFC 9162 v2 publishing contract (see Open Question §13) |

## 4. Per-tenant isolation menu

Layer 3 names the isolation tiers we ship; the measurable cross-tenant grading is deferred to Open Question §13.1.

| Tier | Mechanism | Cross-tenant boundary | Where it sits on the shelf |
|---|---|---|---|
...
The boundary properties below (§5–§11) hold for every tier; the tier picks the substrate, not the invariants.
...
Eight content-keyed classes. Each class maps inbound to a regulator's term; outbound mapping (our class → customer class) is the binding artifact for the contract. Per-tenant data residency (NFR-COMP-13) constrains where any class above PUBLIC may sit on the substrate.
...
Minimal-capability default scope: PUBLIC + INTERNAL only. Processing CONFIDENTIAL or higher requires opt-in configuration (BYOK + customer-managed audit sink); the minimal-config shape is honest about not being a compliance posture.
...
Three modes on the same binary, switched by configuration. NFR-FLEX-15 names this verbatim.
...
Fail-closed: if the egress proxy is unreachable, the Compute plane fails-closed (drop traffic), never fail-open. Same property holds for the IdP → Control plane path: if IdP is unreachable, new sessions are denied; in-flight sessions continue under their existing token until TTL expiry.

Component-spec wiring of each mode lives in `components/<egress-proxy>.md` (Layer 10). Layer 3 describes the property crossing the boundary; Layer 10 implements.
...
| Tenant DEK rotation | ≤90 d (NFR-SEC-04) | ≤90 d (NFR-SEC-04) | NFR-SEC-04 |
| Tenant KEK rotation | ≤365 d (NFR-SEC-04) | ≤365 d (NFR-SEC-04) | NFR-SEC-04 |
| Revoke latency | ≤5 min | ≤5 min | NFR-SEC-04 |
| Per-tenant trust domain | n/a (single-tenant) | per-tenant SPIFFE trust domain | open question — see §13 |
| Internal mTLS substrate | TLS 1.3 on Compose internal network / k8s service-mesh / microVM vsock per substrate (NFR-SEC-37) | same, customer-CA-rooted | NFR-SEC-37 |

NFR-SEC-09 ("SPIFFE SVID on every inter-component call") and NFR-SEC-05 ("customer-CA injected into sandbox trust store") read as full-capability primitives. On the minimal-capability shelf the identity-binding invariant is satisfied by NFR-SEC-26 Ed25519 JWT, and the egress-CA is the auto-generated self-signed cert that the proxy ships. Both NFRs hold in spirit on both shelves; the substrate differs.

### 8.1 Signer identity per boundary

Every boundary that emits a signed artifact names the signer; the signer's key custody and rotation are properties of that boundary, not a separate zone. Tool pick (cert-manager + Vault vs OpenBao PKI vs SPIRE upstream vs smallstep step-ca) is deferred to `arch/adr-pki-tool-pick` (Open Question §13.5).

| Artifact | Signer identity (minimal) | Signer identity (full) | Rotation |
|---|---|---|---|
| Egress JWT (per-session, ≤4h) | Control-plane host-local Ed25519 key | Per-tenant SPIRE-issued workload SVID rooted in customer PKI | ≤90 d signing-key rotation; 24 h overlap; `kid` (NFR-SEC-11) |
...
| Container / image admission | Cosign signature verified against Sigstore Rekor (SLSA L3 per project release rules) | Same; key pinned per customer policy | per release |

Out-of-scope clarification: customer IdP signing keys (OIDC `kid` rotation, SAML metadata) live with the customer per `Customer IdP` actor in §3; this table covers signers we ship.

## 9. Encryption matrix

Zone × class → TLS posture + at-rest posture + key custody. The full matrix lives in the canonical encryption table here; component specs cite this matrix and never restate it.
...
Substrate-specific enforcement of the inter-component TLS invariant — Compose internal network, k8s service-mesh, microVM vsock — is a component-spec choice; the invariant (NFR-SEC-37) is encryption-in-transit at every named inter-component arrow with the two documented carve-outs in §7.

## 10. Audit zone — mandatory in code, pluggable in sinks

Audit-pipeline is **mandatory in code** (NFR-SEC-03 hash-chained; NFR-REL-12 durable bus on critical path; NFR-COMP-01 7-year minimum retention). Audit-pipeline **sinks** are pluggable: file-system at the minimal-capability shelf; OCSF v1.x JSON bridges to customer SIEM (Splunk, Chronicle, Elastic, QRadar, Sentinel, Kafka, S3, syslog) as opt-in additions per NFR-MAINT-AUDIT-SCHEMA.

The pipeline is drawn as our zone; sinks are external actors. The contract between us and the sink is the OCSF v1.x JSON schema plus the bridge transport (see Open Question §13.3).

Tamper-evidence: hash-chained store always; daily Merkle head software-signed by default; HSM-signed Merkle head at the full-capability shelf per NFR-SEC-03. Signing-identity custody per §8.1.
...
`robust(ness)` appears in the EU AI Act Art. 15 title verbatim; the surrounding backticks preserve the regulator phrase against our project banned-vocab list.

## 12. See also

Buffer files under `docs/future-architecture/architecture/` that this document supersedes for the trust-boundary topic; useful as background while the buffer is migrated.

- `02-layer4-control-plane.md` — control-plane shape + RPC surface + LLM proxy semantics (§02 cites; relevant to §2 Control plane row of this doc).
- `04b-credential-broker.md` — credential broker shape, "two planes, two mechanisms" (network identity vs resource authorization) — relevant to §2 Credential broker row and §8.1 broker signer row.
- `05-layer1-guest-agent.md` — guest agent contract — relevant to §2 Compute plane row.
- `07-security.md` — security cross-cutting concerns — relevant to §6, §7, §8.
- `08-networking.md` — networking and egress shape — relevant to §7.
- `10-observability.md` — audit pipeline and bus contract (§02 cites; relevant to §2 Audit pipeline row and §10).

## 13. Open questions

Five tracked per `CLAUDE.md` content-routing §6. Each links a GitHub issue created before this document moves out of `draft`.
...
GitHub issues are created in a follow-up PR before this draft moves to `proposed`.
