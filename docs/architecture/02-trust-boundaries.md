...
| 3 | **Storage broker** | Host-side broker for the guest's mutable user-data mount. Holds the storage-backend credential; the guest mounts through the broker holding only a session-scoped resource handle (a `filesystem_id`), never the backend key ([NFR-SEC-25](manifesto/02-nfrs.md)). Its own backend traffic is authorized at the Egress trust-edge like any upstream. Drawn as a zone distinct from Credential custody: it has a guest-facing interface (the mount) that custody does not, and it governs an inbound data path (read/write into the sandbox) where the Egress trust-edge governs only outbound. Mount substrate (FUSE / virtio-fs / 9p) is a component-spec choice. | [NFR-SEC-25](manifesto/02-nfrs.md) |
...
- Storage broker — [NFR-SEC-25](manifesto/02-nfrs.md), [NFR-SEC-15](manifesto/02-nfrs.md).
...
The mount serves the guest's mutable user-data through the broker. The broker's own backend traffic leaves through the Egress trust-edge like any upstream, so the storage backend is also an egress allow-list destination — but the guest never sees that leg. Both paths share one invariant the table makes concrete: the guest holds a session-scoped handle at most ([NFR-SEC-25](manifesto/02-nfrs.md), [NFR-SEC-23](manifesto/02-nfrs.md)).
