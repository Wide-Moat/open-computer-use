...
Ownership per row is named in [`02-nfrs.md`](manifesto/02-nfrs.md) §"Scope ownership": DELIVER (we ship + are accountable), ENABLE (we publish the contract/telemetry, customer owns the policy), REVISIT (claims more than our scope, marked `[REVISIT — non-gating]` and not enforced by CI).
...
| 2 | **Storage** | The session's mutable user-data and the components that reach it. No in-guest, edge, or storage component holds a signing key: the Control plane mints the weak Storage-JWT and publishes a JWKS the [Egress trust-edge](glossary.md#egress-trust-edge) validates against, the edge exchanges that JWT at the credential authority for the real filestore credential, and the storage engine verifies the injected credential ([NFR-SEC-25](manifesto/02-nfrs.md), [ADR-0013](adr/0013-storage-credential-custody.md), [ADR-0019](adr/0019-egress-exchanges-filestore-credential.md)). The guest holds only a weak session JWT (a `filesystem_id`+workspace+org-scoped bearer, short-lived, delivered at provisioning and held in the mount config); the edge validates it against the Control plane's JWKS, exchanges it at the credential authority for the real filestore credential, and the storage engine enforces the scope on that credential. The **Object-store service** (`ocu-filestore`) is the sole entry point to storage — only it speaks to the storage engine, and it receives the real edge-injected credential, holding no signing key ([ADR-0015](adr/0015-storage-decomposition-by-trust-plane.md)). The **Web UI** (`ocu-webui`) handles file upload, preview, and download on a dedicated file/UI ingress, not the MCP listener ([NFR-SEC-78](manifesto/02-nfrs.md)); an external [Data-plane client](glossary.md#data-plane-client) (§3) reaches it, it verifies a peer-minted embed token and sets a first-party session ([NFR-SEC-82](manifesto/02-nfrs.md)), and an uploaded body is archive-validated and content-classified in a capability-free parser-sandbox before it becomes mount-visible ([NFR-SEC-80](manifesto/02-nfrs.md), [NFR-SEC-81](manifesto/02-nfrs.md)). A per-object `downloadable` tag is a third authorization axis beyond scope and intent (`read` / `write` / `preview`), resolved at read: a non-downloadable object is readable in-session but yields no egress-eligible artifact, and the tag reaches the Egress trust-edge as a deny signal ([NFR-SEC-73](manifesto/02-nfrs.md)). The Object-store service and the Web UI emit OCSF file-activity durable-first: the event commits to a local durable record before fan-out, a downstream sink failure does not deny or stall the file operation, and that local record is the non-repudiation point ([NFR-SEC-79](manifesto/02-nfrs.md)). For multi-tenant deployments storage is instantiated per tenant — one principal per tenant filesystem scope ([NFR-SEC-76](manifesto/02-nfrs.md)). The In-guest mount client (`ocu-rclone-filestore`, Compute plane) presents storage files as a filesystem in the guest and dials the Object-store service; mount substrate (FUSE / virtio-fs / 9p) is a component-spec choice. | [NFR-SEC-25](manifesto/02-nfrs.md) |
...
| Data-plane client (OCU SPA or headless caller) | client → Web UI | file data plane (upload / list / download / preview-render); embed token verified → first-party session ([NFR-SEC-78](manifesto/02-nfrs.md), [NFR-SEC-82](manifesto/02-nfrs.md)) | optional — absent in headless deployments |
...
flowchart LR
    EXT[external actors]
    DPC[Data-plane client]
    CP[Control plane<br/>MCP + operator interfaces]
    OSS[Object-store service<br/>the only door to storage]
    WEBUI[Web UI<br/>upload / preview / download]
    VM[Compute plane<br/>session sandbox + in-guest mount client]
    EDGE[Egress trust-edge]
    SDS[SDS source<br/>static file or customer store]
    ISS[Credential authority<br/>exchange counterparty · external or bundled]
    ENGINE[Storage engine<br/>S3 or local volume · off-box]
    AUDIT[Audit pipeline]
    EXT -- "MCP (agent) · operator API" --> CP
    CP -- "Session JWT (rotated)" --> VM
    CP -- "provisioning push · Control-minted Storage-JWT + ca_cert" --> VM
    VM -- "storage data leg · weak session JWT (guest-out)" --> EDGE
    EDGE -- "validates session JWT (Control JWKS), exchanges, injects real cred" --> OSS
    EDGE -- "token exchange (per-session, cached)" --> ISS
    DPC -- "host leg (does not cross egress)" --> WEBUI
    WEBUI -- "file-op intent" --> OSS
    OSS -- "real injected cred · engine enforces scope" --> ENGINE
    SDS -- "credential over SDS" --> EDGE
    EDGE -- "agent egress · auth injected" --> EXT
    CP & OSS & WEBUI & EDGE -- "OCSF" --> AUDIT
    AUDIT --> EXT
```

Canonical source: [`docs/architecture/diagrams/02-trust-boundaries.mmd`](./diagrams/02-trust-boundaries.mmd) — the canonical file encodes the convention "solid border = always present; dashed border = optional configuration" (CPROXY, SOAR, ICAP, SIEM, KMS, TLOG drawn dashed). The inline block above omits the dashed-vs-solid distinction; the §3 actor table is canonical for optional-vs-required.
...
OCU is an ephemeral workspace and retains no customer file bytes for any class — bytes leave with the session (scrubbed at teardown, [NFR-SEC-65](manifesto/02-nfrs.md)) or go to the customer's store. The only row that is an OCU duty is REGULATED-AUDIT: the 7 y / 10 y floor binds the audit record OCU keeps ([NFR-COMP-01](manifesto/02-nfrs.md), §10), not customer content.
...
Egress posture follows what the deployment needs, not a fixed default ([NFR-FLEX-15](manifesto/02-nfrs.md), [ADR-0007](adr/0007-egress-auth-mechanism.md)). Each rung adds only what the rung above requires; a deployment that needs no authenticated egress carries no certificate authority.
...
Bump is the default *only when an upstream credential is configured*. "One-click" is preserved at the bump rung by automating the CA — generate per deployment, auto-inject the public certificate into the sandbox trust store at start — not by omitting it. DLP-ICAP ([NFR-COMP-28](manifesto/02-nfrs.md)) is a configuration of the bump rung, not a separate rung: an ICAP req-mod / resp-mod hook between the decrypt and re-encrypt steps, with a plaintext segment at the ICAP wire.

Upstream-authorization injection (NFR-SEC-23/27) requires the edge to originate the upstream connection — the L7 property the bump rung provides. The storage leg requires the same L7 property: the edge validates the guest's weak session JWT, exchanges it at the issuer for the real filestore credential, and overwrites the `Authorization` header before originating the connection to the Object-store service ([ADR-0019](adr/0019-egress-exchanges-filestore-credential.md)). Transparent pass-through does not terminate TLS and so cannot attach an upstream credential; it reaches only endpoints needing no upstream authorization. The mechanism that attaches the upstream credential is selected per upstream by [ADR-0007](adr/0007-egress-auth-mechanism.md); v1 ships edge injection only. Upstreams that pin a certificate or require client-mTLS / proof-of-possession cannot be satisfied by edge injection and are tracked at [#176](https://github.com/Wide-Moat/open-computer-use/issues/176).
...
|---|---|---|---|---|
| **Storage mount** | provisioning push (inbound) + data leg (guest-out) | Storage (§2 zone 2) | a weak session [Storage-JWT](glossary.md#storage-jwt) (held in the mount config, presented to the edge) | the Control plane mints the Storage-JWT (holds its signing key, publishes the JWKS the edge validates against); the edge exchanges the session JWT at the credential authority for the real filestore credential; the storage engine verifies and enforces scope on it |
...
The guest's file operations run through the In-guest mount client, which dials the Object-store service `service_url` guest-out over the single egress hop and presents the weak session JWT as a static `Authorization: Bearer` ([ADR-0014](adr/0014-storage-transport-tier-universal-network-leg.md)). The guest reaches the Object-store service, never the storage engine directly. No in-guest, edge, or storage component holds a signing key: the Control plane mints the weak session JWT and publishes the JWKS the edge validates against (delivered into the guest by the Control plane in the provisioning push), and the real filestore credential is issued by a separately-named credential authority over RFC 8693 token exchange — that exchange counterparty may be external/customer-provided (Vault/KMS) or bundled minimal ([ADR-0013](adr/0013-storage-credential-custody.md), [ADR-0019](adr/0019-egress-exchanges-filestore-credential.md)). The egress hop terminates TLS, validates the session JWT against the Control plane's JWKS, exchanges it at the credential authority for the real credential keyed on `filesystem_id`, and overwrites the `Authorization` header with that credential before forwarding to the Object-store service; the exchange is per-session, cached, and the edge holds no signing key — it exchanges ([NFR-SEC-85](manifesto/02-nfrs.md)). Scope is enforced at the storage engine on the real credential: a foreign `filesystem_id` is rejected (403 PermissionDenied — a valid signature but a foreign scope; a missing or expired token is 401) at the engine, not at the hop ([NFR-SEC-31](manifesto/02-nfrs.md)). The guest holds a weak session JWT the backend does not accept on its own, never a signing key ([NFR-SEC-25](manifesto/02-nfrs.md), [NFR-SEC-23](manifesto/02-nfrs.md)).

The mount client opens no uncontrolled outbound path of its own: the data leg traverses the single governed egress hop to reach the Object-store service, and a direct guest dial that bypasses that hop is forbidden ([NFR-SEC-16](manifesto/02-nfrs.md)), because it would be an outbound path the control cannot see; the hop is that control, emitting an OCSF event a compromised guest cannot silence ([NFR-SEC-85](manifesto/02-nfrs.md)).

These are the two **guest** data paths. The Web UI carries a non-guest path (§2 zone 2): an external Data-plane client (§3) reaches the host-side Web UI over the file/UI ingress, and the Web UI calls the Object-store service for the file operation. That path is caller↔Web UI, host-side, not a guest channel, so the host-dials-guest invariant ([NFR-SEC-43](manifesto/02-nfrs.md)) is unaffected; its authorization (embed token, three-axis claim) and inbound-body controls (archive validation, content classification) are the Web UI NFRs anchored in §2 zone 2.
...
Token taxonomy is canonical here; the three classes, their scopes, and their TTLs match [`manifesto/02-nfrs.md`](manifesto/02-nfrs.md) §"Token TTL taxonomy". Each is named with its own scope, TTL, signer, and consumer. The guest holds session-scoped tokens — the Session JWT (control channel) and the weak Storage-JWT (storage leg, held in the mount config). The host-side service-to-service credentials are a separate class (purpose-scoped, possibly several); the specific Control plane ↔ Audit RPC token never reaches the guest. The upstream credential the Egress trust-edge attaches is not an OCU-issued token class: it is delivered over Envoy SDS from a static file (solo) or a customer-provided SDS-compatible store (enterprise), and its scope and TTL are the source's.

| Token class | Scope | TTL | Consumer | §02 anchor |
|---|---|---|---|---|
| **Session JWT** | per session (Control plane → Compute plane); checked on the control channel against a separately-supplied expected container name, not carried as a JWT claim | short-lived, rotated (≤ 60 min is an OCU anti-replay default) | Compute plane (guest agent), proving session identity to the Control plane | NFR-SEC-10 |
| **Storage-JWT** | per session, scoped to one `filesystem_id` + workspace + org | short-lived, fixed-window, no refresh (TTL is an OCU deployment parameter, not a sourced value) | Compute plane (guest), held in the mount config and presented to the Egress trust-edge, which validates it against the Control plane's JWKS and exchanges it for the real filestore credential, so it never reaches the Object-store service or the storage engine; carries no signing key | NFR-SEC-25 |
| **Host-side service-to-service credentials** | inter-component RPC, host-side (a purpose-scoped class, e.g. the Control plane ↔ Audit RPC token, which never reaches the guest) | ≤ 60 min on the Control plane ↔ Audit token | host-side service-to-service | NFR-SEC-23 |

| Property | Minimal shelf | Full shelf | §02 anchor |
|---|---|---|---|
| Inter-component identity | Host-local signing key, checked against a separately-supplied expected container name (not a JWT claim) | Workload identity from customer PKI per tenant | NFR-SEC-26 / NFR-SEC-09 |
...
| Storage | implicit-trust zone (Control-minted weak session JWT, engine-enforced scope on the exchanged credential) | § 500.15(a) encryption + key custody | Art. 28 ICT third-party general | Art. 10 data governance | CEK-08, DSP-01 |
