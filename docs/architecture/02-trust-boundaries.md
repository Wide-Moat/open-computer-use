...
flowchart LR
    ...
    EDGE -- "validates session JWT, exchanges at issuer, injects real cred" --> OSS
...
|---|---|---|---|---|
| **Storage mount** | provisioning push (inbound) + data leg (guest-out) | Storage (§2 zone 2) | a weak session [Storage-JWT](glossary.md#storage-jwt) (held in the mount config, presented to the edge) | off-box credential issuer (signing key, issues both tokens); the edge exchanges the session JWT for the real credential; the storage engine verifies and enforces scope on it |
...
| **Storage-JWT** | per session, scoped to one `filesystem_id` + workspace + org | ~6 h fixed, no refresh | Compute plane (guest), held in the mount config and presented to the Egress trust-edge, which validates and exchanges it for the real filestore credential, so it never reaches the Object-store service or the storage engine; carries no signing key | NFR-SEC-25 |
...
| Storage | implicit-trust zone (off-box-issued scoped bearer, engine-enforced scope) | § 500.15(a) encryption + key custody | Art. 28 ICT third-party general | Art. 10 data governance | CEK-08, DSP-01 |
