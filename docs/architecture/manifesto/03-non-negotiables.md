...
**Kill-switch is unreachable from the agent path.** Session revoke, quota kill, and denylist mutation are not reachable over the path that carries tool-call execution; enforcement is network topology, not an in-process guard ([NFR-SEC-52](02-nfrs.md)). Anti-example: an agent-callable RPC that revokes its own session.
...
**The wire surface is versioned and changes additively.** The Control-plane RPC and the OCSF Published Language are versioned; a breaking change needs a major version and a deprecation window, CI-enforced ([NFR-IC-04](02-nfrs.md), [08-contracts.md](../08-contracts.md) §4). Anti-example: silently changing the tool-call request shape between releases.
