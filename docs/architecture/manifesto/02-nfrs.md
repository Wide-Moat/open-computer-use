...
Six zones interact: Control plane, Credential custody, Storage broker, Compute plane, Egress trust-edge, Audit pipeline. The trust-zone diagram is canonical in [`../02-trust-boundaries.md`](../02-trust-boundaries.md) §5 (source: [`../diagrams/02-trust-boundaries.mmd`](../diagrams/02-trust-boundaries.mmd)). Every NFR below sits on one of those zones.

Two identity primitives carry the inter-zone calls: a session JWT bound to `container_name` (Control plane → Compute plane, TTL ≤ 60 min, rotated), and a network-bound egress identity (Compute plane → Egress trust-edge: the fact that traffic arrived from the sandbox at all is the identity). The guest holds no upstream credential. Upstream authorization is attached at the Egress trust-edge, which fetches the real credential from Credential custody and injects it on the outbound leg; the sandbox sends an unauthenticated request and never sees the key. Component skeletons land under [`../components/`](../components/) when each component spec opens (see [PROCESS.md](../PROCESS.md)).
...
These are token lifetimes (anti-replay windows), not session controls. Session inactivity and absolute re-authentication are separate, regulator-anchored limits — idle ≤15 min (NFR-SEC-40), absolute ≤12 h (NFR-SEC-41) — and the token rotates under both. The custody credential lease bounds how long the Egress trust-edge may hold a fetched upstream credential; the guest holds no upstream credential.
...
- **DELIVER** — we ship the code, we are accountable for the measurable target. Failure = our defect. Example: sandbox escape (NFR-SEC-02), egress proxy with upstream-auth injection (NFR-SEC-05/23), credential custody (NFR-SEC-29), audit pipeline (NFR-SEC-03 reframed for tx-log submission), RTO/RPO of our planes (NFR-REL-01/02/03), encryption defaults (NFR-SEC-33).
...
| NFR-SEC-05 | Single forward-proxy egress (customer-CA injectable; MITM-inspecting is the opt-in mode, NFR-FLEX-15) | Information Disclosure, exfil | single forward proxy; customer-CA injectable into the sandbox trust store; WebSocket support; ext_authz hook; strict upstream TLS validation + fail-closed | integration test | primitives-backlog |
...
| NFR-SEC-23 | Secrets externalised in host-side Credential custody; upstream authorization injected at the Egress trust-edge on the outbound leg, never delivered into the guest; the high-value class (LLM upstream API key) carries tighter TTL on the custody credential lease. The storage-backend credential is held by the Storage broker, not here (NFR-SEC-25) | Information Disclosure | zero secrets in container manifest and zero secrets in the guest (disk, env, memory); ≤60 min generic internal token; ≤15 min custody lease for high-value class; revocable ≤5 min generic, ≤1 min high-value | image scan + in-guest secret scan + integration | [`04b-credential-broker.md`](../../future-architecture/architecture/04b-credential-broker.md) §TTL |
...
| NFR-SEC-25 | No host bind-mount of a secret into the guest, and no secret reaches the guest by any path. A storage mount carries only a session-scoped resource handle (e.g. a `filesystem_id`); the storage backend's real credential is held by the host-side Storage broker (its own trust zone, [`02-trust-boundaries.md`](../02-trust-boundaries.md) §2 zone 3), and the broker's backend traffic is authorized at the Egress trust-edge like any other upstream. Mount substrate (FUSE / virtio-fs / 9p) is a component-spec choice | Information Disclosure | zero host-side secret bind-mounts; zero upstream credential in the guest; the guest holds at most a session-scoped resource handle | runtime audit + in-guest secret scan | [`06-storage.md`](../../future-architecture/architecture/06-storage.md) §mounts + [`04b-credential-broker.md`](../../future-architecture/architecture/04b-credential-broker.md) |
...
| NFR-SEC-27 | Egress identity = **network-bound** — the sandbox has no route out other than the Egress trust-edge; it sends an unauthenticated request and the edge attaches the upstream authorization from custody | Spoofing | `curl` from sandbox reaches an allow-listed upstream and returns 200 with no client cert or token present in the guest; outbound default-route = the edge | network-policy audit + integration test | [`08-networking.md`](../../future-architecture/architecture/08-networking.md) §egress |
...
| NFR-SEC-29 | Credential custody — host-side; holds no master key for compliance-bearing tiers (delegated STS). The Egress trust-edge fetches a scoped credential at injection time; the guest has no channel to custody and never receives a credential. Custody binds the credential to the requesting session, not to a guest-presented token | Information Disclosure, custody compromise | custody credential lease TTL ≤15 min; custody-internal revocation propagation ≤1 min (a tighter subset of the ≤5 min platform-wide target in NFR-SEC-04); no guest-facing custody interface | per-tier deployment audit + custody integration test | [`04b-credential-broker.md`](../../future-architecture/architecture/04b-credential-broker.md) §topology |
...
| NFR-SEC-31 | Per-session filesystem-prefix isolation enforced where the credential is scoped — at the Egress trust-edge / custody boundary, against the session identity, not against a guest-presented token | Information Disclosure | cross-session reads architecturally impossible, not policy-guarded | edge / custody integration test | [`04b-credential-broker.md`](../../future-architecture/architecture/04b-credential-broker.md) §scope |
...
| NFR-MAINT-09 | Mutation-testing score on auth / sandbox / audit / custody packages | ≥60% per release | mutation-testing report attached to release | `CLAUDE.md` CI ruleset |
