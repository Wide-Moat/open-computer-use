...
| NFR-SEC-08 | Egress allow-list — the customer declares which outbound destinations are reachable; everything else is denied | Information Disclosure, lateral | Egress trust-edge enforces a customer-declared allow-list, deny-by-default, with auditable denials. Destination type (LLM API, MCP server, object store, internal API) is not a separate control — each is one allow-listed destination | SIEM-side check | primitives-backlog |
