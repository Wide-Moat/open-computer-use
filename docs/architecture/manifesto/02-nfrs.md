...
Five zones interact: Control plane, Credential custody, Compute plane, Egress trust-edge, Audit pipeline. The trust-zone diagram is canonical in [`../02-trust-boundaries.md`](../02-trust-boundaries.md) §5 (source: [`../diagrams/02-trust-boundaries.mmd`](../diagrams/02-trust-boundaries.mmd)). Every NFR below sits on one of those zones.
...
Three short-lived token classes carry three different lifetimes by design. Only the session JWT reaches the guest; the other two are host-side and never enter the sandbox.
...
| NFR-SEC-23 | Secrets externalised in host-side Credential custody; upstream authorization injected at the Egress trust-edge on the outbound leg, never delivered into the guest; high-value classes (LLM upstream API key, cloud-storage SigV4 origination) carry tighter TTL on the custody credential lease | Information Disclosure | zero secrets in container manifest and zero secrets in the guest (disk, env, memory); ≤60 min generic internal token; ≤15 min custody lease for high-value class; revocable ≤5 min generic, ≤1 min high-value | image scan + in-guest secret scan + integration | [`04b-credential-broker.md`](../../future-architecture/architecture/04b-credential-broker.md) §TTL |
