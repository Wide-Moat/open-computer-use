...
Categories follow ISO/IEC 25010:2023 plus two inserted sections (Compliance, Cost) that have no native ISO mapping. Every row carries a measurable target or a `tbd` placeholder with a tracking-issue slug — no row ships with a marketing adjective. Section caps follow `CLAUDE.md`: this file ≤400 lines.

## Trust boundaries

Three planes meet at the sandbox. Every NFR below sits on one of them.

```mermaid
flowchart LR
    CP[control-plane]
    BROKER[credential broker<br/>real cloud creds<br/>HOST-side]
    PROXY[MITM egress-proxy<br/>our CA in sandbox trust store<br/>default-deny + IP/SNI allowlist<br/>metadata-only logging<br/>ext_authz hook]
    VM[microVM per session<br/>PID 1 = guest agent Rust<br/>NO real cloud creds<br/>NO docker.sock<br/>/mnt/uploads ro · /mnt/outputs rw]
    NET[internet / S3 / API]

    CP -- "Ed25519 JWT on WebSocket<br/>claim=container_name" --> VM
    BROKER -- "FUSE scoped JWT<br/>filesystem_id" --> VM
    VM -- "default-route<br/>tool / browser / curl" --> PROXY
    PROXY -- "strict TLS validation<br/>fail-closed" --> NET
```

Three identity primitives, three planes. Inbound carries Ed25519 JWT on WebSocket bound to `container_name`. Data access carries scoped JWT through the broker bound to `filesystem_id`. Egress identity is **network-bound** — the fact that traffic arrived from the sandbox at all (single path through MITM-proxy) is the identity. Component skeletons live in [`docs/future-architecture/architecture/`](../../future-architecture/architecture/).
...
| NFR-FS-03 | LLM-request byte-identical round-trip for cached request shapes (proxy semantics, not provider semantics) | CI integration test asserts | release pipeline | [`02-layer4-control-plane.md`](../../future-architecture/architecture/02-layer4-control-plane.md) §LLM proxy |
...
| NFR-PERF-03 | Session-create p99, cold-start (kata-fc — our planned untrusted tier) | ≤2 s | k6 perf gate | [`10-observability.md`](../../future-architecture/architecture/10-observability.md) §SLO |
...
| NFR-PERF-11 | Egress-proxy throughput ceiling | `tbd` (`arch/egress-proxy-throughput-ceiling`) | — | open |
...
| NFR-REL-04 | LLM upstream failover | ≤5 min unhealthy → secondary | chaos test | DORA Art. 29 |
...
| NFR-SEC-02 | Sandbox escape | EoP, Tampering | microVM-default (Kata-FC for untrusted tier, Kata-CH where virtio-fs/GPU needed) + seccomp BPF + Landlock; zero red-team pass per release | red-team suite | EU AI Act Art. 15(5) |
| NFR-SEC-03 | Audit-log tamper | Tampering, Repudiation | hash-chained append-only audit log; daily Merkle head over the chain published to a transparency log within 24 h. Default tier: Merkle head signed by software key (tamper-detection without hardware). Bank tier: Merkle head signed via HSM (PKCS#11 / KMIP) — hardware-rooted, tamper-resistant even under host compromise | daily transparency-log probe; HSM-signature check on bank-tier | RFC-9162 + EU AI Act Art. 12 |
...
| NFR-SEC-08 | MCP allow-list enforcement | Information Disclosure, lateral | egress gateway enforces customer allow-list; deny-by-default; auditable denials | SIEM-side check | primitives-backlog |
...
| NFR-SEC-21 | EU AI Act Art. 15(5) candidate test suite | Tampering, Information Disclosure | pass/fail per release; four named attack classes (data poisoning, model poisoning, adversarial examples, confidentiality attacks); mitigation thresholds tracked against forthcoming EU Commission implementing act | red-team suite | EU AI Act Art. 15(5) verbatim |
...
| NFR-SEC-25 | In-guest mount initiation; no host bind-mount of secrets into guest; covers filesystem AND upstream-API-key classes | Information Disclosure | zero host-side secret bind-mounts; FUSE backend initiated from inside guest | runtime audit | [`06-storage.md`](../../future-architecture/architecture/06-storage.md) §mounts + [`04b-credential-broker.md`](../../future-architecture/architecture/04b-credential-broker.md) |
...
| NFR-SEC-28 | Wide-Moat ships a default image and a FIPS-validated image variant. Default uses Apache-2.0 crypto providers and supports the full modern cipher set (TLS 1.3, ChaCha20, Ed25519). FIPS variant uses FIPS 140-3 validated crypto modules and may carry a narrower cipher floor (e.g. TLS 1.2). Security envelope is equivalent — FIPS variant adds regulator-acceptable certification, not stronger crypto. Two-image stance rationale tracked in `arch/adr-fips-binary-tier` | (compliance) | per-release default + FIPS variants emitted; FIPS-conformance test green on FIPS variant | release-pipeline | separate-binary precedent across enterprise OSS |
| NFR-SEC-29 | Credential broker — per-VM; bound to loopback / vsock / UDS only; broker holds no master key for compliance-bearing tiers (delegated STS); in microVM tiers binding = vsock CID + JWT scope (CID is hypervisor-assigned, unforgeable by guest code) | Information Disclosure, broker compromise | scoped-JWT TTL ≤15 min; revocation propagation ≤1 min; public-interface bind admission-rejected | per-tier deployment audit + broker integration test | [`04b-credential-broker.md`](../../future-architecture/architecture/04b-credential-broker.md) §topology |
...
| NFR-SEC-35 | Host kernel floor + microVM kernel cmdline hardening | EoP, kernel CVEs | host kernel ≥5.10 + KVM present (Kata requirement); every microVM-tier template kernel boots with `init_on_free=1`, `nomodule`, `random.trust_cpu=1`, `panic=1` | Helm pre-install hook + image-spec audit | [`04-layer2-runtimes.md`](../../future-architecture/architecture/04-layer2-runtimes.md) §kernel |
...
| NFR-MAINT-02 | Upgrade rollback to N-1 | ≤30 min control-plane; ≤5 min data-plane node | DR exercise | DORA Art. 12 |
...
| NFR-FLEX-02 | Sandbox runtime fallback — kata-fc / kata-ch / gVisor / runc all buildable from same artefact | per-release artifact inventory | release pipeline | [`04-layer2-runtimes.md`](../../future-architecture/architecture/04-layer2-runtimes.md) §tiers |
| NFR-FLEX-03 | Identity provider portability — SAML 2.0 + OIDC + SCIM 2.0 + SPIFFE + LDAP/AD bind-mode; tested against Okta + Entra ID + Ping + ForgeRock + IBM Verify; platform always the relying-party — no in-house JWT issuer / SCIM endpoint / user table with passwords | per-release integration test | CI lint + integration | NYDFS Part 500 + [`gaps.md`](../../future-architecture/gaps.md) B |
...
| NFR-FLEX-06 | Same egress invariant across Docker Compose + k8s + microVM | one invariant, three thin wrappers | cross-substrate test | [`design-notes.md`](../../future-architecture/design-notes.md) DN-1 |
...
| NFR-FLEX-12 | Customer-tenant data-plane as first-class deployment shape — control-plane carries metadata only; data-plane (compute + storage + logs) runs in customer tenant | customer-hosted data-plane option green per release | per-release integration | primitives-backlog (customer-tenant data-plane) |
...
| NFR-FLEX-15 | Egress posture tier-selectable — MITM-inspecting (default, for SIEM/DLP) OR transparent pass-through (for client-cert-pinning workloads); single CA-trust-store invariant applies only to MITM tier | per-tier integration test | release pipeline | [`08-networking.md`](../../future-architecture/architecture/08-networking.md) §egress tiers |
...
| NFR-COMP-07 | Evidence-as-code bundle per release — immutable + attached; ≤7 d to TPRM intake; includes the latest available SOC 2 attestation (Type I from GA, Type II from GA+12 months), SIG Lite + Core, CAIQ v4, CSA STAR L2, ISO 27001 cert, pen-test exec summary, DORA RoI template row, CycloneDX SBOM, SLSA L3 provenance, VEX. SOC 2 and ISO 27001 evidence claims require the bank-tier audit binding (HSM-signed Merkle head per NFR-SEC-03 bank tier); the default-tier (software-signed Merkle) is for solo / non-regulated deployments and does not carry the attestation | bundle present per release; field-level completeness gate | release-pipeline | primitives-backlog (evidence-as-code) |
...
| NFR-COMP-09 | Post-market monitoring data flow (EU AI Act Art. 72) — per-release plan + telemetry collectable | release notes | per-release | EU AI Act Art. 72 |
| NFR-COMP-10 | DPIA / FRIA refresh per major release + on substantial change | template review | per-release | GDPR Art. 35 + EU AI Act Art. 26 |
...
| NFR-COMP-14 | EU AI Act Art. 15 accuracy declaration (qualitative) — every model deployment publishes accuracy metric + methodology | release notes | per-release | EU AI Act Art. 15(1) + 15(3) verbatim |
| NFR-COMP-15 | SR 26-2 carve-out — platform governance hooks provide customer's parallel-framework evidence for agentic AI. SR 26-2 evidence pack requires bank-tier audit binding (NFR-SEC-03 bank tier); default-tier deployments are out-of-scope for SR 26-2 attestation | per-release evidence bundle includes logging + accountability + escalation | TPRM-pack audit | SR 26-2 verbatim |
...
| NFR-COMP-18 | ISO/IEC 42001:2023 AI Management System conformance | scope statement + cert in evidence bundle | per-release | ISO/IEC 42001:2023 |
...
| NFR-COMP-20 | Per-component status page — minimum 4 named components (control-plane, sandbox-runtime, egress-proxy, audit-pipeline) with independent uptime ≥90 d + Atom/RSS subscription + email/webhook channels minimum | live per component | per-release | industry-baseline (Atlassian Statuspage convention) |
...
| NFR-COMP-25 | ZDR contractual-clause checklist per supported managed LLM upstream | presence-gate per release | CI gate | per-upstream ZDR docs (cited at release) |
| NFR-COMP-26 | Configurable prompt-redaction filter before any external LLM call; per-tenant policy; redaction events audited | release pipeline | integration test | NYDFS Industry Letter (training-data clause) |
...
| NFR-COMP-29 | PAM — just-in-time admin elevation via SAML-asserted attributes; integration with industry PAM brokers; no shared service accounts in production | per-release audit | integration test | NIST 800-53 AC-2(7) |
...
5. Substrate-specific TLS enforcement choice (k8s service-mesh vs Compose-network vs microVM-vsock) — component-spec level decision. Track: `arch/intra-platform-tls-substrate-choice`.
