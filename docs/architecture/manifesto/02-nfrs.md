...
|---|---|---|---|---|
| Session JWT | per session (user sees one continuous interaction) | ≤4 h | guest (proves session identity to Control plane) | NFR-SEC-10 |
...
| NFR-SEC-10 | Per-session session JWT (session-identity, Control plane → guest; not an upstream credential) | Replay, token-theft | `exp ≤ 4 h` per token; while the session is active the Control plane issues a fresh token before expiry (rotation, not extension — a stolen token dies in ≤4 h); session length is not capped by the token TTL (a hard session cap, if any, is a separate policy — [#178](https://github.com/Wide-Moat/open-computer-use/issues/178)) | token-lifetime test | [`antipatterns.md`](../../future-architecture/antipatterns.md) A8 |
