...
| NFR-SEC-38 | Workload-trust profile declaration — deployment configuration declares `workload_trust_profile ∈ {trusted_operator, internal_workforce, untrusted}`. Platform validates the configured runtime tier matches the declared profile at admission time; mismatch is a hard error. Allowed pairings: `trusted_operator` → runc, gVisor, microVM; `internal_workforce` → gVisor, microVM (runc rejected); `untrusted` → microVM only (microVM is post-v1, so `untrusted` is not deployable in v1 GA — admission rejects with a pointer to [`arch/microvm-tier-v1.1`](https://github.com/Wide-Moat/open-computer-use/issues/161)). Per-session profiles tracked for v1.1+ at [`arch/per-session-trust-profile`](https://github.com/Wide-Moat/open-computer-use/issues/162) | Spoofing, Tampering | Helm pre-install hook + control-plane startup check; CI gate on Helm chart values schema; 9-cell admission test matrix (6 valid v1 + 3 rejected v1 pending microVM tier). Status: commitment; implementation lands when control-plane code exists | per-release admission test fixture | this PR + [`RESEARCH-sandbox-tier-workload-driven.md`](../../future-architecture/research/RESEARCH-sandbox-tier-workload-driven.md) §Q5 |
| NFR-SEC-39 | Tier-downgrade alarm — a deployment previously on gVisor or microVM is reconfigured to runc, OR `workload_trust_profile` is downgraded (e.g. `internal_workforce` → `trusted_operator`) on a deployment with at least one active session | Tampering, Repudiation | audit event `config.trust_profile.downgraded` emitted on the audit bus within ≤30 s of the reconfiguration commit; SIEM-bridge classification = HIGH; SOAR webhook payload conformant to NFR-COMP-27 signal schema. Status: commitment; implementation lands when audit pipeline + SOAR integration exists | integration test against the audit pipeline asserts event emitted, SIEM classification HIGH, SOAR webhook fired within 30 s | this PR + [`RESEARCH-sandbox-tier-workload-driven.md`](../../future-architecture/research/RESEARCH-sandbox-tier-workload-driven.md) §Q5 |
...
- AP-13 — picking the sandbox tier by data classification (e.g. tagging a workload as NPI and inferring that it therefore requires the microVM tier, or tagging it as PUBLIC and inferring runc is fine). Container-escape attack surface for adversarial LLM-issued code is identical regardless of data class. Data class governs retention (NFR-COMP-01), key custody (NFR-SEC-33), and residency (NFR-COMP-13); the workload-trust profile picks the runtime tier. Forbidden by NFR-SEC-02 + NFR-SEC-38 + the "Sandbox tier — workload-driven selection" sub-section. Enforced at the prose level by a vale rule on `(NPI|RESTRICTED|CONFIDENTIAL) → (microVM|hardware-virt)` phrasing.
