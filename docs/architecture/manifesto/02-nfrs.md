...
| NFR-SEC-10 | Per-session session JWT (session-identity, Control plane → guest; not an upstream credential) | Replay, token-theft | `exp ≤ session-max (4 h)`; refresh issues new token, never extends | token-lifetime test | [`antipatterns.md`](../../future-architecture/antipatterns.md) A8 |
...
| NFR-SEC-29 | Credential custody — host-side; holds no master key for compliance-bearing tiers (delegated STS). The Egress trust-edge fetches a scoped credential at injection time; the guest has no channel to custody and never receives a credential. Custody binds the credential to the requesting session, not to a guest-presented token | Information Disclosure, custody compromise | custody credential lease TTL ≤15 min; revocation propagation ≤1 min; no guest-facing custody interface | per-tier deployment audit + custody integration test | [`04b-credential-broker.md`](../../future-architecture/architecture/04b-credential-broker.md) §topology |
