...
| GCP `signJwt` (IAM Credentials) | RS256-only and forces a Google `iss` + JWKS the storage engine must trust — wrong key type and trust anchor for the off-box issuer envelope | OpenBao Transit (`ecdsa-p256` / `jws`) as the reference signer; customer KMS as a drop-in over the mint contract |
| AWS STS as the storage-credential issuer | The killed SigV4 / `F8` lease-pull shape ([#205](https://github.com/Wide-Moat/open-computer-use/issues/205), [ADR-0005](../adr/0005-egress-credential-delivery-envoy-sds.md)) — a per-operation request signer, not a pre-issued scoped bearer; reintroduces an OCU minting/STS-delegator surface ADR-0013 removes | Pre-issued ES256 `Authorization: Bearer`, scope enforced at the storage engine; no OCU STS leg on the data path |
