...
| OpenBao Transit (Control-plane Storage-JWT signer: `ecdsa-p256` as an OCU default, `marshaling_algorithm=jws`) | MPL-2.0 (signer engine); the ~100-line OCU JWS assembler is FSL-1.1-Apache-2.0 library code | bundled (the Control plane mints the weak Storage-JWT with its own signing key, T1) | [ADR-0013](../adr/0013-storage-credential-custody.md) |
...
| SPIRE (as the Storage-JWT signer) | Cannot reproduce the scoped envelope the Control plane mints — SVID shape, not a `{filesystem_id + workspace + org}` claim set with the 3-axis `AuthorizationMetadata` and fixed-window no-refresh TTL ([ADR-0013](../adr/0013-storage-credential-custody.md)) | OpenBao Transit signer + OCU JWS assembler as the Control-plane-local signer (T1); a customer KMS over the documented signer contract |
| GCP `signJwt` (IAM Credentials) | RS256-only and forces a Google `iss` + JWKS the edge must trust — wrong key type and trust anchor for the Control-plane signer envelope | OpenBao Transit (`ecdsa-p256` / `jws`) as the reference signer; customer KMS as a drop-in over the signer contract |
| AWS STS as the storage-credential signer | The killed SigV4 / `F8` lease-pull shape ([#205](https://github.com/Wide-Moat/open-computer-use/issues/205), [ADR-0005](../adr/0005-egress-credential-delivery-envoy-sds.md)) — a per-operation request signer, not a pre-issued scoped bearer; reintroduces an OCU minting/STS-delegator surface ADR-0013 removes | Pre-issued `Authorization: Bearer` (an OCU-chosen signing algorithm) minted by the Control plane, validated and exchanged at the edge; no OCU STS leg on the data path |
