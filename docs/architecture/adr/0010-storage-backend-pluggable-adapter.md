...
Addresses Information Disclosure on the backend leg: the object-store service is the sole speaker of the backend protocol to the storage engine. The guest mount client carries the off-box-issued Storage-JWT as a static bearer over the single TLS-terminating egress hop to the service, and the storage engine enforces scope ([ADR-0013](0013-storage-credential-custody.md), [ADR-0016](0016-egress-baseline-inspection-hop-backend-scope.md)). The bearer is static, so the egress hop terminates TLS and forwards it unchanged; there is no per-request signature to preserve. A local-volume engine opens no network leg, so the in-transit obligation is vacuous for it.
