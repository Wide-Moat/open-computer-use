...
- **Teardown rides the runtime-interface seam [ADR-0017](0017-control-plane-repo-boundary.md) already names** (the executor depends on a runtime interface, not the concrete runtime). Its per-substrate teardown mechanics — which graceful-stop each substrate issues, the finalizer predicate held constant across them — are component-05 design ([`05-session-sandbox.md`](../components/05-session-sandbox.md) Shelf delta), not a new boundary. The force-kill-bypasses-the-drain consequence: the cooperative phase rides the runtime graceful-stop, so an unconditional remove that skips it leaves the guest no drain window.
