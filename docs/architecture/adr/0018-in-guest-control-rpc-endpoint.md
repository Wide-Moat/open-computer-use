...
- **The v1 verb set is a single `shutdown`** (wire tag `Shutdown`), an advisory cooperative fast-path. Teardown is host-side. On the bare docker/runc/runsc tier this ADR scopes, v1 teardown is an unconditional force-remove — SIGKILL, no SIGTERM, no cooperative drain window; a SIGTERM/grace window exists only when an orchestrator supplies one (`terminationGracePeriodSeconds`) or the host first issues this advisory verb. Either way the host-driven ordered finalizer (NFR-SEC-65) revokes, route-drops, scrubs, and kills regardless of whether any drain ran. `Shutdown` can at most open or advance the cooperative SIGTERM phase the guest knows how to run; it can never reorder, substitute for, or mark-complete the finalizer, and the finalizer's kill subsumes any drain the guest skips — so the verb is optional, a pre-SIGTERM advance-grace hint only. It is idempotent and host-caller-only, and carries no body — there is no session or `container_name` field to forge. A page-cache-drop maintenance op is a distinct verb from cooperative shutdown; if ever wanted it is a separate `x-ocu-tbd-verbs` member (snapstart family), never folded into `Shutdown`.
...
- **Teardown rides the runtime-interface seam [ADR-0017](0017-control-plane-repo-boundary.md) already names** (the executor depends on a runtime interface, not the concrete runtime). Its per-substrate teardown mechanics — the bare-container force-remove, the orchestrator delete-with-grace, the finalizer predicate held constant across them — are component-05 design ([`05-session-sandbox.md`](../components/05-session-sandbox.md) Shelf delta), not a new boundary. The cooperative drain is a property of the path that opens it (an orchestrator grace window or this advisory verb), not of the bare-container teardown, which force-removes; the finalizer (NFR-SEC-65) is what every path guarantees.
...
None. The cooperative `shutdown` reply needs no separate deadline ([#290](https://github.com/Wide-Moat/open-computer-use/issues/290) resolved): the runtime graceful-stop window bounds the cooperative phase (a `ContainerStop` timeout on the container tiers, `terminationGracePeriodSeconds` under an orchestrator), and the host-driven finalizer (NFR-SEC-65) is the unconditional backstop — the host blocks at most for the grace window, never indefinitely on a guest reply.
