...
The custody ADR ([storage-credential-custody](0013-storage-credential-custody.md)) fixes who holds the storage signing key: an off-box host-side issuer mints a scoped, time-bounded bearer; the control plane delivers it; the guest forwards it unmodified; no in-guest part holds a signing key. With custody pinned off-box, the single-credential argument that justified the weld no longer holds — the planes can be cut by their counterparty without moving any key.
...
| **Object-store service** | the storage engine (engine adapter + `filesystem_id`→prefix + multipart) | no (capability-free; the off-box issuer signs, the guest forwards) | — |
