...
The custody ADR ([storage-credential-custody](0013-storage-credential-custody.md)) fixes who holds the storage signing key: an off-box host-side issuer mints a scoped, time-bounded bearer; the control plane delivers it; the guest holds it as a weak session JWT the Egress trust-edge validates and exchanges at the issuer for the real filestore credential ([ADR-0019](0019-egress-exchanges-filestore-credential.md)); no in-guest part holds a signing key. With custody pinned off-box, the single-credential argument that justified the weld no longer holds — the planes can be cut by their counterparty without moving any key.
...
| **Object-store service** | the storage engine (engine adapter + `filesystem_id`→prefix + multipart) | no (capability-free; the off-box issuer signs; the guest holds a weak session JWT the edge exchanges) | — |
