...
- **Body render — the browser.** Preview render runs in the browser, in a null-origin sandboxed iframe: a `sandbox` attribute carrying `allow-scripts` and **never** `allow-same-origin`. This is a distinct, stricter directive class from the SPA's `frame-ancestors` policy, which governs who frames the SPA, not what an artifact body may do. A null-origin frame cannot reach the embedder's origin, so it also closes the `postMessage` exfil leg without setting COOP — provided the SPA's `message` listener rejects `event.origin === "null"`, which is what such a frame posts. That check was incidental when nothing ran in the frame; it is load-bearing now that a renderer does.
  ...
  Two documents with two policies meet in that frame, and conflating them is the mistake to avoid. The **artifact body**, served from the content route, keeps `default-src 'none'; script-src 'none'; object-src 'none'` — those bytes stay inert wherever they land. The **renderer document** is our own asset, served same-origin into the frame, and it carries the egress block NFR-SEC-86 requires: `default-src 'none'` with a self-only `script-src`/`style-src`, and no `connect-src`, `media-src`, `font-src` or `prefetch-src`. Four directives are opened to `blob:` and to nothing else — `img-src`, `object-src`, `frame-src`, and `style-src` beside `'self'` — because a renderer must display the bytes it was handed and `default-src 'none'` denies even those. A blob URL names no host, so each grant re-admits the artifact already in hand and no network origin. Every one was measured on both sides before it was granted: without it the frame refuses a blob it created itself, and with it a NETWORK request through the same directive still does not reach an attacker sink. What each buys: an image, a PDF in the browser's own viewer, the nested frame HTML renders in, and a renderer's own stylesheet. `default-src 'none'` is the load-bearing half for that class — each of those falls back to it, as do `worker-src`, `child-src`, `manifest-src` and `object-src`, so a permissive fallback would silently re-open every one. The document directives do NOT fall back and are therefore named outright: `base-uri 'none'` and `form-action 'none'`. Leaving `base-uri` to the fallback is the easy mistake — it does not inherit, and an unset one lets renderer script plant a `<base href>` that re-points every relative URL built afterwards at an attacker. The sandbox token list is exactly `allow-scripts`: no `allow-popups`, no `allow-top-navigation`, no `allow-downloads`, no `allow-forms`, no `allow-modals`.
...
- The blob grants are bounded by measurement, not by intent. After all four, the ten egress channels were re-run against the shipped route in a real browser: nine blocked — fetch, XHR, sendBeacon, image beacon, form post, `window.open`, `<base href>`, Worker, prefetch — and only same-frame self-navigation reached the sink, which is the residual this ADR already states. The count did not move when the grants were added, which is the property that matters: a grant that widened egress would have shown up as a tenth channel.
- Fidelity is bounded too, and named rather than implied. A presentation renders at extraction fidelity — text per slide, in document order — not as slide geometry. Reproducing the deck means a layout engine over untrusted drawing instructions, which is the parser surface this ADR exists to bound; the PoC spent a 576 KB viewer plus a charting library on it. A diagram renders the geometry its model states rather than through the vendor's viewer, which the policy refuses from a CDN by construction.
