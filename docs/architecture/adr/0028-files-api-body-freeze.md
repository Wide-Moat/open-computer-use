...
- The list envelope the object-store service emits carries a `next_cursor` the public type drops; the client paginates on `?after=<last_id>`. ADR-0023 already says "opaque-id cursors", which is the `last_id` model, so the extra field is the odd one out.
...
- **List envelope.** `{data, has_more, first_id, last_id}` with opaque-id cursors; the forward cursor is `?after=<last_id>`. `next_cursor` is off the wire.
...
This amends ADR-0023: the FileObject dialect drops `downloadable`, `scope`, and `checksum_md5` from the wire, and the "opaque-id cursors" wording is fixed to the `last_id` forward cursor with no `next_cursor` field. The keystone, the durable handle home, and the five verbs are unchanged.

## Consequences

- Component [04](../components/04-object-store-service.md): the north handler serves the frozen bodies; the create response reuses the metadata FileObject builder, so one object serves create, metadata, and each list item. The `next_cursor` field leaves the wire.
...
- **Add `next_cursor` to the public list type.** Rejected: two cursor models on one envelope, against the ADR-0023 opaque-id wording; the `last_id` forward cursor the client already uses is one model, so the extra field is dropped.
