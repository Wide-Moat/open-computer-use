...
- **A bare `last_id` as the forward cursor (no `next_cursor`).** Rejected (2026-07-02 correction; briefly the ruling): the durable store's created-at-primary keyset walk cannot resume from a bare id — a deleted boundary record repeats or strands a record. Re-ordering the store by file id alone would trade away chronological listing; a per-page id-to-tuple lookup breaks on the same deleted boundary. The opaque token is the only resume key that survives it.
