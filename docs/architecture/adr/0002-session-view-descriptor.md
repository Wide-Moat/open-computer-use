...
Fixes how OCU's Web UI discovers its views, so adding the deferred live-session views (browser, terminal) is additive, not a breaking change.
...
- Positive: the files surface is served by the **Web UI** ([component 08](../components/08-web-ui.md)), its own deployable ([`05-c4-container.md`](../05-c4-container.md) §3, decomposition per [ADR-0015](0015-storage-decomposition-by-trust-plane.md)). The shell that renders the descriptor list reads per-surface sources; the deferred live-view tabs read off the **Session sandbox** host edge, a separate container.
