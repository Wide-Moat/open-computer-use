...
- Positive: the files surface is served by the **Web UI** ([component 08](../components/08-web-ui.md)), its own deployable ([`05-c4-container.md`](../05-c4-container.md) §3, decomposition per [ADR-0015](0015-storage-decomposition-by-trust-plane.md)); the shell that renders the descriptor list is a thin Web UI shell over per-surface sources, not owned by any one container — the deferred live-view tabs read off the **Session sandbox** host edge, a different container.
