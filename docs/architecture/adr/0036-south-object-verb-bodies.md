...
- **`listFiles`** returns the ADR-0028 list envelope over `FileObject` entries, resuming on the same opaque `next_cursor`. The cursor stays opaque on this plane too: the durable store's keyset cursor carries the `(created_at, file_id)` boundary tuple, and a bare id cannot resume that walk. Ordering is the store's created-at-primary total order, and the dialect carries no order parameter on either face; newest-first is a dialect change ratified as an ADR-0028 amendment, never a plane-local knob.
