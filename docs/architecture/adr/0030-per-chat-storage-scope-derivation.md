...
- **Control persists the binding.** Control persists the session-`effective_scope` binding in the session registry (the binding it is already the sole custodian of) so the Web-UI file client and the sandbox tool client resolve a chat's scope through Control's caller-scoped status verb (audience-scoped: a caller resolves only its own chat's scope). The binding lives on the enriched read row, not the frozen core.
- **South credential is the isolation boundary; North F9 is a cooperative shape guard.** The per-chat separation D5 enforces is on the mint/south path: two chats derive two distinct scopes, Control mints each into its own Storage-JWT claim, and the edge and engine key on the validated claim, so a chat's guest holds a credential scoped to its own derived scope and cannot read another chat's objects. This closes the cross-chat file-visibility path by construction. The North F9 leg is different: the embed token attests the tenant namespace base, not a chat. The object-store north face hardens the requested scope for shape and traversal only (it refuses an empty, non-clean, or traversal-shaped id at the edge, before the engine); it does NOT pin the derivation alphabet and does NOT authorize per-chat, because per-chat north enforcement would require the chat identity on an attested channel (an embed-token claim) or a filestore-to-Control lookup, neither of which exists in v1. The daemon's lazy-scaffold decorator is the layer that recognizes a legal derived scope of the deployment base (`base` or `base-<hex>`) and provisions it; a scope that is neither is passed through and refused by the engine, not auto-provisioned. Within-tenant north separation is therefore client-cooperative, matching the reference per-chat-directory behaviour and the deferred boundary below.
...
- Component [04](../components/04-object-store-service.md): the north face gains a shape-and-traversal guard on the requested scope (accept `base` or `base-<hex>`; reject a traversal-shaped id), NOT a per-chat authorization point - within-tenant north separation is cooperative (a north caller supplies its own scope on the trusted F9 channel). The scope-id format is pinned to one engine-legal segment. The engine interface and implementations are unchanged; the daemon lazily scaffolds a shape-validated derived scope's `uploads/`/`outputs/` markers on first access using the existing provisioning verbs, so an agent-writes-first flow to a fresh derived scope does not fault on absent markers. The per-chat credential the south path keys on is the real isolation boundary. Negative: the north face gains scope-shape validation and the daemon gains a lazy-scaffold decorator.
...
Closes the cross-chat file-visibility path on the sandbox/south credential path: two chats of one deployment derive distinct scopes, so each chat's guest holds a Storage-JWT scoped to its own derived scope and cannot reach another chat's uploads or outputs (cross-chat negative test on the derived scope). The North file-pane leg is hardened for scope shape and traversal, not per-chat authorized; within-tenant north separation stays client-cooperative until the chat id rides an attested channel (Open questions, item 1).
