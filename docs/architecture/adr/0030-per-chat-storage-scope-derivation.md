...
- Component [04](../components/04-object-store-service.md): the north face gains a shape-and-traversal guard on the requested scope (accept `base` or `base-<hex>`; reject a traversal-shaped id), NOT a per-chat authorization point - within-tenant north separation is cooperative (a north caller supplies its own scope on the trusted F9 channel). The scope-id format is pinned to one engine-legal segment. The south engine is unchanged - the scope is opaque and already enforced (invariant 4); the per-chat credential the south path keys on is the real isolation boundary. Negative: the north face gains scope-shape validation.
