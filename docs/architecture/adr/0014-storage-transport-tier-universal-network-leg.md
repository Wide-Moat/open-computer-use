...
- `SOC2-CC6.1`: scope and confidentiality on the leg rest on the backend's filesystem_id-claim validation and TLS, recorded where the credential custody is recorded ([ADR-0013](0013-storage-credential-custody.md)), not on transport reachability.
- `DORA-Art.28`: the backend leg is a declared outbound dependency reachable over one governed hop, recordable in the register of information.
...
Re-anchors the storage-leg direction in [06-threat-model.md](../06-threat-model.md) §3 (P4-T1/I2/S1) onto a guest-out network dial, off the prior host-dialled drawing. The exec/control channel's no-guest-reachable-network invariant ([NFR-SEC-43](../manifesto/02-nfrs.md)) and host-peer accept ([NFR-SEC-76](../manifesto/02-nfrs.md)) stay scoped to F6; the storage leg's deny rests at the Egress trust-edge hop and at the backend filesystem_id binding, not on the exec channel's attribution.

This ADR is blocked by [ADR-0013](0013-storage-credential-custody.md) — it cites that ADR's backend `filesystem_id`-claim scope as the leg's deny — and is sequenced with [ADR-0016](0016-egress-baseline-inspection-hop-backend-scope.md) for the single-governed-hop reading of NFR-SEC-16. All three land in the same coordinated PR; this ADR cannot land while the broker-signs / deny-by-default-allow-list canon is still live, or its NFR-SEC-16 citation would silently re-affirm the reversed model.
