...
- Neutral: [05-c4-container.md](../05-c4-container.md) §4 splits the single F7 arrow into a host→guest provisioning push and a guest-out data leg; the container count is a then-true observation, not an invariant, and this split adds no container. The mount contract is one config shape — top-level `service_url` + `ca_cert_pem` + a `mounts[]` array, each mount carrying its own `auth_token` and `readonly` posture — not a two-variant `GuestMountConfig`/`ProvisionMountConfig` split; that split is retired and the per-mount scoped JWT reaches the guest in the same config.
