...
The read-surface adds no mutating reach to the operator plane — the import-boundary test (mirroring NFR-SEC-26) makes a read handler that reaches destroy/revoke/denylist/quota a build failure, so the P2 control-plane anchors in [`06-threat-model.md`](../06-threat-model.md) hold unchanged. The browser never touches `ocu-control`: the BFF is the only hop, holds the UDS dial, and dials as the operator-scoped peer the plane attests (NFR-SEC-76). The shared-password and operator-sees-all limitations are accepted trades of the read-only console, recorded here, not defects.
