...
v1 implements edge-inject. Protocol-broker is named, abstraction-ready, and deferred: no in-deployment component holds an upstream signing key, so the broker is an unbuilt v1 mechanism, not an existing zone. The Storage zone ([02-trust-boundaries.md](../02-trust-boundaries.md) §2) is not that broker — the guest holds a weak session JWT the Control plane minted and the storage engine does not accept on its own, the egress edge validates it against the Control plane's JWKS and exchanges it at a separately-named credential authority (which may be external/customer-provided) for the real filestore credential keyed on `filesystem_id` ([ADR-0019](0019-egress-exchanges-filestore-credential.md)), and scope is enforced at the storage engine on the injected credential; the edge holds no signing key, so it mints nothing. A future scoped-credential upstream that needs per-operation authorization is the case that adds the broker; v1 builds none.
