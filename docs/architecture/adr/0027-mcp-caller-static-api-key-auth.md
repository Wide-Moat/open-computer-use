...
- **Storage.** Salted SHA-256 (`sha256(salt‖secret)`, per-key salt). Record: `{key_id, key_hash, salt, tenant, deployment, expires_at, status, created_at}`. The Control state store on the full shelf; a root-owned hashed-entries file on the minimal shelf. The unsalted hash is rejected (pass-the-hash exposure).
