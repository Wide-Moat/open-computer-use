...
| Scope enforcement | storage engine validates the JWT claim; foreign scope → 403 PermissionDenied from origin (missing/expired token → 401) | egress-side per-destination / per-claim scope at the hop |
