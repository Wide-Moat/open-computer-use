...
The off-box service that holds the sole [Storage-JWT](#storage-jwt) signing key (ES256). It mints a scoped token before sandbox boot; the [Control / operator API](#control--operator-api) delivers the pre-signed token in the provisioning push and never signs ([ADR-0013](./adr/0013-storage-credential-custody.md)). On the minimal shelf the signer is a bundled OpenBao Transit engine; on the full shelf the customer provides it (OpenBao / Keycloak / KMS) over a documented mint contract. No component below it holds the signing key — not the guest, the [in-guest mount client](#in-guest-mount-client), the [Object-store service](#object-store-service), or the [Web UI](#web-ui).
