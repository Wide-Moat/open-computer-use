...
The only door to create or manage a session, repo `ocu-control`. It exposes the operator/lifecycle interface (lifecycle, quota, kill-switch) reached through the [Operator console](#operator-console) or CLI, never over MCP. It dials into the guest (host dials guest); there is no agent-path edge that reaches the kill-switch, denylist, or lifecycle. It holds the [Storage-JWT](#storage-jwt) signing key, mints the weak Storage-JWT at provisioning, delivers it in the provisioning push, and publishes a JWKS the [Egress trust-edge](#egress-trust-edge) validates against ([ADR-0013](./adr/0013-storage-credential-custody.md)). One instance per deployment.
...
Two issuers sit on the storage path. The weak [Storage-JWT](#storage-jwt) is issued by the [Control / operator API](#control--operator-api): the Control plane holds the Storage-JWT signing key, mints the scoped weak Storage-JWT before sandbox boot, delivers it in the provisioning push, and publishes a JWKS the [Egress trust-edge](#egress-trust-edge) validates against (the signing algorithm is an OCU implementation default, e.g. ES256) ([ADR-0013](./adr/0013-storage-credential-custody.md)). The real filestore credential is issued by a separately-named RFC 8693 exchange counterparty, via token-exchange, when the [Egress trust-edge](#egress-trust-edge) presents the weak JWT as the `subject_token` ([ADR-0019](./adr/0019-egress-exchanges-filestore-credential.md)); that counterparty may be external/customer-provided — on the minimal shelf a bundled OpenBao, on the full shelf the customer's authority (OpenBao / Keycloak / KMS) over a documented exchange contract. No component below the Control plane holds the Storage-JWT signing key — not the guest, the [in-guest mount client](#in-guest-mount-client), the [Egress trust-edge](#egress-trust-edge), the [Object-store service](#object-store-service), or the [Web UI](#web-ui).
...
Per-session session-identity token issued by the Control plane to the guest agent, checked against a separately-supplied expected container name on the same channel (not carried as a JWT claim), rotated while the session is active (≤ 60 min is an OCU anti-replay default). It proves session identity to the Control plane; it is not an upstream credential and never leaves toward an upstream. It is the guest's control-channel identity token; on the storage leg the guest separately holds the weak [Storage-JWT](#storage-jwt) (a distinct credential, held in the mount config). The TTL is an anti-replay window, not a session length — session idle (≤15 min, NFR-SEC-40) and absolute (≤12 h, NFR-SEC-41) limits are separate. Distinct from the SDS-delivered upstream credential (attached by the Egress trust-edge, never the guest) and the generic internal RPC token (TTL ≤ 60 min, inter-component, host-side).
...
One member of the host-side service-to-service credential class — a class of purpose-scoped inter-component RPC credentials, possibly several, not one generic token. The member named here authenticates the Control plane ↔ Audit pipeline RPC, TTL ≤ 60 min on that token. This specific Control↔Audit RPC token never reaches the guest and carries no operator scope or upstream credential. Distinct from the [Session JWT](#session-jwt) (guest-held, per session) and the SDS-delivered upstream credential (attached by the Egress trust-edge).
