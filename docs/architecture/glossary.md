...
The scoped ES256 bearer the [in-guest mount client](#in-guest-mount-client) presents to the [Object-store service](#object-store-service). Minted by the [Credential issuer](#credential-issuer), scoped to `{filesystem_id, workspace, org}` with a three-axis authorization (scope, intent, downloadable), fixed ~6 h TTL, no in-session refresh, no re-sign. Delivered into the guest mount config at provisioning and forwarded unmodified as an opaque static `Authorization: Bearer`. Scope is validated at the [storage engine](#storage-engine) — a foreign `filesystem_id` under the same token returns 403 PermissionDenied — a valid signature but a foreign scope; a missing or expired token is 401 ([ADR-0016](./adr/0016-egress-baseline-inspection-hop-backend-scope.md)). A leaked token reaches at most this filesystem for the remaining TTL; it is not a whole-backend key.
